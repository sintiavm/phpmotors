<?php
   ...
   //create a funciton that render items from a vehicle id to a details page
    function buildVehicleDetails($invInfo, $thumbInf ){
        $dv = '<div class="inv-details">';
        $dv .= "<img src='$invInfo[imgPath]' alt='Image of $invInfo[imgName] $invInfo[invModel] on phpmotors.com'>";
        $dv .= '<hr>';
        ...
        foreach ($thumbInf as $image) {
            $dv .= "<img src='$image[imgPath]' alt='Image of $invInfo[imgName] $invInfo[invModel] on phpmotors.com'>";
        }
        $dv .= '</div>';
        return $dv;
    }
